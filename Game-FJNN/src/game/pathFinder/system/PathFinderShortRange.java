	...
	public void findPath(PathRequest request) {		
				...
				for(PathDirections d: PathDirections.values()){					
					...
					if(nextLocation.getY() > 0 && nextLocation.getY() < map.getHeight()
						...
						if(nextLocation.getX() == end.getX() && nextLocation.getY() == end.getY()){
							found = true;
							break;
						}else if(this.map.entityCanAcces(request.getEntity(), nextLocation.getX(), nextLocation.getY())){
		...
		if(!found){
			...
			while(lastLocation.getDistance()>0){
				...
				for(PathDirections d: PathDirections.values()){	
					int px = pathDataLocation[0]+maxDistance+d.getX();
					int py = pathDataLocation[1]+maxDistance+d.getY();
					if(px > 0 && py > 0 && px <= pathData.length && py <= pathData[px].length){
						PathLocation nextLocation = pathData[px][py];

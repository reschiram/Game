...
public class LobbyEM implements ToServerMessageEventListener{
	...
	private void handleEvents(ToServerMessageEvent event) {
		...
		if(send) {
			for (Player player : lobby.getConnectedPlayers()) {
				if (player.hasCatchedUp() && player.getValidatedUser().getServerClientID() != event.getClientID()) {
					try {
						if(event.getMessage().getId() != GameSPM.DataPackage_PlayerMoved) System.out.println("SendPackage: "+event.getMessage().getId());
						gameSM.getServerManager().sendMessage(player.getValidatedUser().getServerClientID(), DataPackage.getPackage(event.getMessage()));
	...
	private void handleEntityRequestMessages(ToServerMessageEvent event) {
		if (event.getMessage().getId() == GameSPM.DataPackage_EntityCreationRequest_Drone) {
			...
			System.out.println("recieveDroneRequest-OldRequestID: " + oldRequestID);
			...
			System.out.println("recievePlayerRequest-OldRequestID: " + oldRequestID);
			if (oldRequestID < 0) {
	...
	public void publishRequest(long clientID, ServerEntityRequest request, int oldRequestID, int currentClientRequestID) {
		...
		for (Player player : lobby.getConnectedPlayers()) {
			if (player.getValidatedUser().getServerClientID() == clientID) {
				try {
					PackageType message = gameSM.getGameSPM().answerRequest(clientID, request, currentClientRequestID);
					gameSM.getServerManager().sendMessage(player.getValidatedUser().getServerClientID(), DataPackage.getPackage(message));
					
					this.lobby.getPlayer(clientID).addKnownEntity(request.getServerEntity());
				} catch (Exception | InvalidServerClientIDException e) {
					e.printStackTrace();
				}
			} else if (player.hasCatchedUp() && oldRequestID < 0 && !player.knowsEntity(request.getServerEntity())) {
				try {

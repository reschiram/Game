...
//		if(res.isGround())System.out.println("update");
		...
		int state = 0;
		Mapdata currentData = Map.getMap().getChunks()[x/Map.DEFAULT_CHUNKSIZE][ y   /Map.DEFAULT_CHUNKSIZE].getMapData(x, y  , res.isGround())[0];
		if(currentData!=null && (!res.isGround() || currentData.getResource().getID()!=MapResource.Air_Background.getID()))state += 1;
		currentData = Map.getMap().getChunks()[x/Map.DEFAULT_CHUNKSIZE][(y-1)/Map.DEFAULT_CHUNKSIZE].getMapData(x, y-1, res.isGround())[0];
		...
		currentData = Map.getMap().getChunks()[x/Map.DEFAULT_CHUNKSIZE][ y  /Map.DEFAULT_CHUNKSIZE].getMapData(x, y  , res.isGround())[0];
